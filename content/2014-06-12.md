...
In that approach, whenever there is a change, the old content of the document is added as an attachment.  This has a few key strengths:
...
To glue this all together, I propose a couple of conventions on top of the original approach.

## Naming as version number

You can name attachments whatever you like, including adding what looks like path information.  I propose to store attachments using the convention `versions/NUMBER`.

This allows me to figure out what the next version number in the series should be without loading individual attachments.  It also lends itself to easy natural ordering.

## History includes the present

We need to store the current version in the change history.  This is slightly redundant for records in which the published version is also the latest version, but is key in adding support for unpublished changes.

## Edits only at the end

As a reminder, we want the document itself to represent the latest published version.  There may be later unpublished versions, but the proposed changes are chained.
...
## Figuring out where the current document sits in history

To make this work, we need some sense of where the current document sits in the document history.  I propose adding our own separate `version` field that is used internally to track changes and not exposed to end users.  That field should match the attachment name (again, `versions/NUMBER`).
...
We may not choose to build a UI that relies on all of those features, but the simple naming convention gives us all of those options.
...
If the `?attachments=true` parameter is passed, the results will also include MIME-encoded attachment content, so we can get everything we care about in a single request.
