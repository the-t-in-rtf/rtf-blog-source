...
For convenience, we store the document version twice in the history.  Once in the document itself, and once in the name of the attachment, as in `versions/NUMBER`.

Having the version in the list of attachments allows us to see where we are in history without loading the version history.  It also allows us to figure out what the next version number in the series should be without loading the version history.  It also lends itself to easy natural ordering of edits in both directions.
